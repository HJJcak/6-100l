...
def get_frequencies(input_iterable):
    ...
    for element in input_iterable:
        count = 0
        for element_c in input_iterable:
            if element_c == element:
                count += 1
        frequency_dic[element] = count
...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    """
    diff, all = 0.0, 0.0
    nodup_list = list(set(list(freq_dict1.keys()) + list(freq_dict2.keys())))

    for element in nodup_list:
        if element in freq_dict1.keys() and element in freq_dict2.keys():
            diff += abs(freq_dict1[element] - freq_dict2[element])
            all += freq_dict1[element] + freq_dict2[element]
        elif element in freq_dict1.keys() and element not in freq_dict2.keys():
...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    for element in nodup_list:
        count = 0
        if element in freq_dict1.keys() and element in freq_dict2.keys():
            count += freq_dict1[element] + freq_dict2[element]
        elif element in freq_dict1.keys() and element not in freq_dict2.keys():
            count += freq_dict1[element]
        else:
            count += freq_dict2[element]
        if count >= max:
            nodup_dic[element] = count
            max = count
    
    for element, score in nodup_dic.items():
        if score == max:
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    ...
    word_freq_dic = get_frequencies(file_list)

    tf_dic = {element:(freq / len(file_list)) for element, freq in word_freq_dic.items()}
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    for file_path in file_paths:
            file_string = load_file(file_path)
            file_list = text_to_list(file_string)
            all_file.append(file_list)
            all_words += file_list
    all_words_nodup = sorted(list(set(all_words)))

    for word in all_words_nodup:
        count = 0
        for file in all_file:
            if word in file:
                count += 1
        idf_dict[word] = math.log10(len(all_file) / count)
...
def get_tfidf(tf_file_path, idf_file_paths):
    ...
    for word, tf_score in tf_dict.items():
        if word in idf_dict.keys():
            tfidf_list.append((word, tf_score * idf_dict[word]))
    
    return sorted(tfidf_list, key=lambda x:x[0])
...
if __name__ == "__main__":
    ...
    # world, friend = text_to_list(hello_world), text_to_list(hello_friend)
    # world_word_freq = get_frequencies(world)
    #f riend_word_freq = get_frequencies(friend)
